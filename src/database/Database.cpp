...
bool Database::checkAndExpire(const std::string &key) {
  ...
  if (it->second.expiryTimeMs != -1 &&
      getCurrentTimeMs() >= it->second.expiryTimeMs) {
    data.erase(it);
    return false;
  }
  return true;
...
Response Database::set(const std::string &key, const std::string &value,
                       long long ttlMs) {
...
Response Database::ttl(const std::string &key) {
  if (!checkAndExpire(key))
    return {Status::KEY_NOT_FOUND, "Key not found",
            (long long)-2}; // -2 for missing key
  ...
  return {Status::OK, "TTL found",
          timeRemainingMs / 1000}; // returning TTL in seconds
}
...
// Lists
Response Database::lpush(const std::string &key, const std::string &value,
                         long long ttlMs) {
  long long expiry = (ttlMs == -1) ? -1 : getCurrentTimeMs() + ttlMs;
...
Response Database::rpush(const std::string &key, const std::string &value,
                         long long ttlMs) {
  long long expiry = (ttlMs == -1) ? -1 : getCurrentTimeMs() + ttlMs;
  checkAndExpire(key); // clear if expired
...
Response Database::lset(const std::string &key, long long index,
...
Response Database::lrange(const std::string &key, long long start,
                          long long stop) {
  if (!checkAndExpire(key)) {
