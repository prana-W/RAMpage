...
private:
    ...
    // Helper to check and remove expired keys. Returns true if key is valid (not expired/deleted).
...
public:
    // Core & Strings
    Response set(const std::string& key, const std::string& value, long long ttlMs = -1);
    ...
    Response rpush(const std::string& key, const std::string& value, long long ttlMs = -1);
    ...
    Response lset(const std::string& key, long long index, const std::string& value);
